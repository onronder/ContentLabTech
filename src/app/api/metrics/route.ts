...
import { getRequestAnalytics, getActiveRequests } from '@/lib/monitoring/performance-middleware';
  ...
  try {
    ...
    switch (type) {
      ...
      case 'endpoint':
        ...
        const endpointMetrics = metricsCollector.getMetricsForEndpoint(endpoint, method || undefined);
        return NextResponse.json({
          endpoint,
          method: method || 'ALL',
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid metrics type. Valid options: system, summary, time-window, endpoint, active-requests, performance' },
          { status: 400 }
        );
    }
  } catch (error) {
