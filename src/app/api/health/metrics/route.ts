  ...
  try {
    ...
    const metrics = {
      ...
      incidents: {
        ...
        escalations: errorMetrics.alertingMetrics.escalations,
      },
      compliance: {
...
function formatPrometheusMetrics(metrics: any): string {
  ...
  // Response time
  lines.push(`# HELP response_time_milliseconds Average response time in milliseconds`);
  lines.push(`# TYPE response_time_milliseconds gauge`);
  lines.push(`response_time_milliseconds ${metrics.performance.responseTime.average}`);
  ...
  for (const check of metrics.healthChecks) {
    lines.push(`# HELP health_check_status Health check status (1=healthy, 0.5=degraded, 0=unhealthy, -1=unknown)`);
    lines.push(`# TYPE health_check_status gauge`);
    lines.push(`health_check_status{name="${check.name}",category="${check.category}",id="${check.id}"} ${statusMap[check.status as keyof typeof statusMap] || -1}`);
  }
...
function formatCSVMetrics(metrics: any): string {
  ...
  // Summary metrics
  lines.push(`${timestamp},health_score,summary,${metrics.summary.healthScore},score,${metrics.summary.overallHealth}`);
  lines.push(`${timestamp},availability,sla,${metrics.availability.current},percent,${metrics.availability.current >= 99.9 ? "healthy" : "degraded"}`);
  lines.push(`${timestamp},response_time,performance,${metrics.performance.responseTime.average},milliseconds,healthy`);
  lines.push(`${timestamp},throughput,performance,${metrics.performance.throughput.current},rpm,healthy`);
  lines.push(`${timestamp},error_rate,performance,${metrics.performance.errorRate.current},percent,healthy`);
  
  // Capacity metrics
  lines.push(`${timestamp},cpu_usage,capacity,${metrics.capacity.cpu.current},percent,${metrics.capacity.cpu.current < 80 ? "healthy" : "degraded"}`);
  lines.push(`${timestamp},memory_usage,capacity,${metrics.capacity.memory.current},percent,${metrics.capacity.memory.current < 85 ? "healthy" : "degraded"}`);
  lines.push(`${timestamp},disk_usage,capacity,${metrics.capacity.disk.current},percent,${metrics.capacity.disk.current < 90 ? "healthy" : "degraded"}`);
  
  // Individual health checks
  for (const check of metrics.healthChecks) {
    lines.push(`${timestamp},${check.name.replace(/,/g, "_")},${check.category},${check.responseTime},milliseconds,${check.status}`);
  }
