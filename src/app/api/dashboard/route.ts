  ...
  try {
    ...
    if (!projectId || !name) {
      return createErrorResponse(
        "Project ID and dashboard name are required",
        400
      );
    }
    ...
    const validatedWidgets = widgets.map(widget => {
      ...
      if (!widget.id || !widget.type || !widget.title) {
        throw new Error(
          `Invalid widget configuration: missing required fields`
        );
      }
    ...
    if (fetchError) {
      ...
      return createErrorResponse(
        "Dashboard created but failed to fetch details",
        500
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isShared) filters.isShared = isShared;
    if (isTemplate) filters.isTemplate = isTemplate;
    if (search) filters.search = search;

    const supabase = await createClient();

    // Build query
    let query = supabase.from("custom_dashboards").select(`
        *,
        project:projects (
          id,
          name,
    ...
    const transformedDashboards = (dashboards || []).map(dashboard => {
      ...
      return {
        ...
        widgets: (
          (dashboardData["widgets"] as DashboardWidgetData[]) || []
        ).map((widget: DashboardWidgetData) => ({
          id: widget.widget_id,
        ...
        permissions: {
          canEdit: dashboardData["created_by"] === user.id,
