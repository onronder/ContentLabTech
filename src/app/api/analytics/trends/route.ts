...
interface TrendsRequest {
  ...
  params: {
    ...
    contentIds?: string[];
    timeframe?: number;
    metric?: 'pageviews' | 'organic_traffic' | 'conversion_rate' | 'engagement' | 'all';
    ...
    switch (action) {
      case 'trends': {
        ...
        if (params.contentId) {
          ...
          result = {
            ...
            historical: processAnalyticsData(analytics || [], params.metric, params.granularity),
      ...
      case 'comparison': {
        if (!params.contentIds?.length || params.contentIds.length < 2) {
          return createErrorResponse('At least 2 content IDs required for comparison', 400);
        }
        ...
        const comparisonData = params.contentIds.map(contentId => {
          ...
          const contentAnalytics = analytics?.filter(a => a.content_id === contentId) || [];
          
          return {
            contentId,
            title: content?.title || 'Unknown',
            data: processAnalyticsData(contentAnalytics, params.metric, params.granularity),
            summary: calculateSummaryStats(contentAnalytics, params.metric),
          };
        });
      ...
      case 'forecast': {
        ...
        const { data: forecast, error: forecastError } = await supabase.functions.invoke(
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (contentId) {
      ...
      if (contents?.length) {
        query = query.in('content_id', contents.map(c => c.id));
      } else {
        return NextResponse.json({
          trends: [],
...
function processAnalyticsData(
  ...
  return Object.values(grouped).map((item: any) => ({
    ...
    avg_session_duration: item.count > 0 ? item.avg_session_duration / item.count : 0,
  }));
}
...
function aggregateAnalyticsByDate(analytics: any[], metric?: string): any[] {
  const dailyTotals = analytics.reduce((acc, item) => {
    ...
    acc[date].bounce_rate_sum += (item.bounce_rate || 0) * (item.sessions || 1);
    acc[date].duration_sum += (item.avg_session_duration || 0) * (item.sessions || 1);
    acc[date].count += 1;
  ...
  return Object.values(dailyTotals).map((item: any) => ({
    ...
    conversion_rate: item.sessions > 0 ? (item.conversions / item.sessions) * 100 : 0,
    bounce_rate: item.sessions > 0 ? item.bounce_rate_sum / item.sessions : 0,
    avg_session_duration: item.sessions > 0 ? item.duration_sum / item.sessions : 0,
  }));
}
...
function generateTrendPredictions(predictionData: any, timeframeDays: number): any[] {
  ...
  for (let i = 1; i <= Math.min(timeframeDays, 30); i++) {
    ...
    const dailyPageviews = (predictionData.predictions?.pageviews?.predicted || 0) / timeframeDays;
    const dailyTraffic = (predictionData.predictions?.organicTraffic?.predicted || 0) / timeframeDays;
