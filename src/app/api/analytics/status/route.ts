...
export async function GET(request: NextRequest) {
  try {
    ...
    if (projectId) {
      ...
      const responseData = {
        ...
        summary: {
          ...
          completedJobs: projectJobs.filter(j => j.status === 'completed').length,
          failedJobs: projectJobs.filter(j => j.status === 'failed').length,
          processingJobs: projectJobs.filter(j => j.status === 'processing').length,
    ...
    if (error instanceof AppError) {
      ...
      if (error.details.category === 'database' || error.details.category === 'external_service') {
        const projectId = context.additionalData?.projectId as string;
        if (projectId) {
          const degradationResult = await gracefulDegradationManager.handleAnalyticsFailure(
            projectId,
            error
          );
          ...
          return createGracefulResponse(error, {
            fallbackData: {
              ...
              queueStats: { total: 0, pending: 0, processing: 0, completed: 0, failed: 0, processing_capacity: 1 },
              summary: { totalJobs: 0, completedJobs: 0, failedJobs: 0, processingJobs: 0, pendingJobs: 0 },
              fallbackMode: true,
 ...
 */
async function getAnalysisResultsCached(supabase: unknown, projectId: string) {
  ...
  try {
    ...
    let contentAnalysis = analyticsCache.get(projectId, CacheKeys.CONTENT_ANALYSIS);
    if (!contentAnalysis) {
      ...
      if (contentResults?.[0]) {
        ...
        analyticsCache.set(projectId, CacheKeys.CONTENT_ANALYSIS, contentAnalysis);
      }
    }
    ...
    let competitive = analyticsCache.get(projectId, CacheKeys.COMPETITIVE_INTELLIGENCE);
    if (!competitive) {
      ...
      if (competitiveResults?.[0]) {
        competitive = {
          ...
          strategicRecommendations: competitiveResults[0].strategic_recommendations,
          lastUpdated: competitiveResults[0].created_at,
        };
        
        // Cache with longer TTL for competitive data
        analyticsCache.set(projectId, CacheKeys.COMPETITIVE_INTELLIGENCE, competitive);
      }
    }
    ...
    let industryBenchmark = analyticsCache.get(projectId, CacheKeys.INDUSTRY_BENCHMARKING);
    if (!industryBenchmark) {
      ...
      if (benchmarkResults?.[0]) {
        ...
        analyticsCache.set(projectId, CacheKeys.INDUSTRY_BENCHMARKING, industryBenchmark);
      }
    }
 ...
 */
async function getAnalysisResultsFromCache(projectId: string): Promise<Record<string, unknown>> {
  const results: Record<string, unknown> = {};
  
  // Try to get data from cache
  const contentAnalysis = analyticsCache.get(projectId, CacheKeys.CONTENT_ANALYSIS);
  ...
  const competitive = analyticsCache.get(projectId, CacheKeys.COMPETITIVE_INTELLIGENCE);
  const industryBenchmark = analyticsCache.get(projectId, CacheKeys.INDUSTRY_BENCHMARKING);
