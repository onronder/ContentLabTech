...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!projectId || !title || !content) {
      return createErrorResponse('Project ID, title, and content are required', 400);
    }
    ...
    const seoScore = calculateSEOScore(title, content, meta_description, focus_keywords);
