...
const COMMENT_TYPES = [
  ...
  { value: 'approval', label: 'Approval', icon: ThumbsUp, color: 'text-green-600' },
  { value: 'concern', label: 'Concern', icon: AlertCircle, color: 'text-red-600' },
  { value: 'insight', label: 'Insight', icon: Zap, color: 'text-purple-600' },
];

export function CollaborativeWorkspace({ projectId, sessionId, analysisId }: CollaborativeWorkspaceProps) {
  ...
  const loadSession = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const currentSession = sessions.find((s: CollaborativeSession) => s.id === sessionId);
  ...
  const createSession = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewSessionData({ sessionName: '', description: '', participants: [] });
      } else {
  ...
  const getCommentTypeIcon = (type: string) => {
    ...
    return <Icon className={`h-4 w-4 ${commentType?.color || 'text-gray-600'}`} />;
  };
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge variant="outline" className="text-xs">
                ...
                <span className="ml-1">{getCommentTypeLabel(comment.commentType)}</span>
              </Badge>
            ...
            <div className="flex items-center gap-2">
              ...
              {!comment.isResolved && (comment.commentType === 'question' || comment.commentType === 'concern') && (
                <Button
                  variant="ghost"
                  size="sm"
  ...
  const renderActivity = (activity: SessionActivity) => {
    ...
    return (
      <div key={activity.id} className="flex gap-3 p-2 text-sm text-muted-foreground">
    ...
    return (
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Create a new session to collaborate with your team on content analysis
          </CardDescription>
        </CardHeader>
            ...
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {session.participants.slice(0, 5).map(participantId => {
                  ...
                  return (
                    <Avatar key={participantId} className="h-8 w-8 border-2 border-white">
                      <AvatarImage src={member?.avatar} />
                      <AvatarFallback className="text-xs">
                        {member?.fullName?.charAt(0) || member?.email?.charAt(0) || '?'}
                      </AvatarFallback>
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {session && formatDistanceToNow(new Date(session.createdAt), { addSuffix: true })}
                  </span>
                </div>
              </CardContent>
            </Card>
