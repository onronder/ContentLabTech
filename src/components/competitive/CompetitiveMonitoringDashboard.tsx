...
export function CompetitiveMonitoringDashboard({ projectId }: CompetitiveMonitoringProps) {
    ...
    <div className="space-y-6">
      ...
      {unreadAlerts.length > 0 && (
        <Alert variant={criticalAlerts.length > 0 ? 'destructive' : 'default'}>
          ...
          <AlertDescription>
            ...
            {criticalAlerts.length > 0 && `, including ${criticalAlerts.length} critical alert${criticalAlerts.length !== 1 ? 's' : ''}`}
          </AlertDescription>
        </Alert>
      )}
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          {loading ? (
            <div className="text-center py-8">Loading competitive data...</div>
          ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold">
                          {analyses.length > 0 
                            ? Math.round(analyses.reduce((sum, a) => sum + a.visibility.averagePosition, 0) / analyses.length)
                            : 0
                          }
                        </p>
                      </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold">
                          {analyses.reduce((sum, a) => sum + a.visibility.estimatedTraffic, 0).toLocaleString()}
                        </p>
                      </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold">
                          {analyses.reduce((sum, a) => sum + a.opportunities.length, 0)}
                        </p>
                      </div>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium">{analysis.visibility.estimatedTraffic.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Keywords</p>
                          <p className="font-medium">{analysis.visibility.keywordCount}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Avg Position</p>
                          <p className="font-medium">{analysis.visibility.averagePosition.toFixed(1)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Opportunities</p>
                          <p className="font-medium">{analysis.opportunities.length}</p>
                        </div>
                      </div>
                      <Progress value={analysis.visibility.visibilityScore} className="h-2" />
                    </CardContent>
                  </Card>
                ))}
              </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {ranking.change !== 0 && (
                              <div className={`flex items-center gap-1 ${getChangeColor(ranking.change)}`}>
                                ...
                                <span className="text-xs">{Math.abs(ranking.change)}</span>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    {analysis.opportunities.map((opportunity, index) => (
                      <div key={index} className="p-3 rounded border">
                        <div className="flex items-start justify-between">
                          ...
                          <div className={`text-right ${getOpportunityColor(opportunity.potential)}`}>
