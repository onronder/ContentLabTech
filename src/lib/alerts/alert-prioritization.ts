...
export class AlertPrioritizationEngine {
  ...
  calculatePriorityScore(alert: CompetitiveAlert): number {
    ...
    const hoursSinceAlert =
      (Date.now() - alert.timestamp.getTime()) / (1000 * 60 * 60);
  ...
  ): PrioritizedAlert["businessContext"] {
  ...
  scheduleDelivery(
    ...
    if (emailChannel && preferences.channels.email.frequency !== "immediate") {
      return this.getNextScheduledTime(
        now,
        preferences.channels.email.frequency
      );
    }
  ...
  private getNextBusinessHour(
    ...
    const [hours, minutes] = quietHours.end.split(":").map(Number) as [
      number,
      number,
    ];
  ...
  prioritizeAlerts(
    ...
    return alerts
      .map(alert => {
        ...
        const deliveryChannel = this.getDeliveryChannels(
          priorityLevel,
          preferences
        );
        ...
        const scheduledDelivery = this.scheduleDelivery(
          prioritizedAlert,
          preferences
        );
        if (scheduledDelivery) {
          prioritizedAlert.scheduledDelivery = scheduledDelivery;
        }
  ...
  private areAlertsRelated(
    ...
    const timeDiff = Math.abs(
      alert1.timestamp.getTime() - alert2.timestamp.getTime()
    );
    ...
    const sharedEntities = alert1Entities.filter(e =>
      alert2Entities.includes(e)
    );
   ...
   */
  private determineClusterCategory(
    alerts: PrioritizedAlert[]
  ): AlertCluster["category"] {
  ...
  private calculateAggregatedPriority(alerts: PrioritizedAlert[]): number {
    ...
    const totalWeight = alerts.reduce(
      (sum, alert) => sum + alert.priorityScore / 100,
      0
    );
