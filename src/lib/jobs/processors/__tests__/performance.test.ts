...
describe('PerformanceAnalysisProcessor', () => {
  ...
  describe('validate', () => {
    ...
    it('should reject invalid device types', () => {
      mockJob.data.params.devices = ['invalid-device'];
      const isValid = processor.validate(mockJob.data);
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('estimateProcessingTime', () => {
    it('should estimate time based on pages, locations, and devices', () => {
      const time = processor.estimateProcessingTime(mockJob.data);
      // 3 pages × 2 locations × 2 devices × 20 seconds = 240 seconds
      expect(time).toBe(240);

      // Test with single device
      mockJob.data.params.devices = ['desktop'];
      const singleDeviceTime = processor.estimateProcessingTime(mockJob.data);
      expect(singleDeviceTime).toBe(120); // Half the time
    });
  });
  ...
  describe('process', () => {
    ...
    it('should provide device-specific analysis', async () => {
      ...
      result.data.deviceComparison.forEach(device => {
        expect(['desktop', 'mobile', 'tablet']).toContain(device.device);
        ...
        expect(device.metrics.cumulativeLayoutShift).toBeGreaterThanOrEqual(0);
      });
    });
    ...
    it('should generate meaningful performance recommendations', async () => {
      ...
      result.data.recommendations.forEach(rec => {
        ...
        if (rec.potentialSavings) {
          ...
          if (rec.potentialSavings.score) {
            expect(rec.potentialSavings.score).toBeGreaterThan(0);
          }
        }
      });
    });
    ...
    it('should identify performance opportunities', async () => {
      ...
      if (result.data.opportunities) {
        ...
        result.data.opportunities.forEach(opp => {
          expect(opp.type).toBeDefined();
          expect(opp.title).toBeDefined();
          expect(opp.description).toBeDefined();
          expect(opp.savings).toBeDefined();
          
          if (opp.savings.ms) {
            expect(opp.savings.ms).toBeGreaterThan(0);
          }
          if (opp.savings.bytes) {
            expect(opp.savings.bytes).toBeGreaterThan(0);
          }
        });
      }
    });
    ...
    it('should show mobile performance is typically slower than desktop', async () => {
      ...
      const desktopData = result.data.deviceComparison.find(d => d.device === 'desktop');
      const mobileData = result.data.deviceComparison.find(d => d.device === 'mobile');
      
      if (desktopData && mobileData) {
        // Mobile should generally have slower metrics
        expect(mobileData.metrics.largestContentfulPaint).toBeGreaterThanOrEqual(desktopData.metrics.largestContentfulPaint);
        expect(mobileData.metrics.speedIndex).toBeGreaterThanOrEqual(desktopData.metrics.speedIndex);
      }
    });
  });
  ...
  describe('score calculation', () => {
    it('should prioritize Core Web Vitals in scoring', async () => {
      ...
      if (coreWebVitals.lcp < 2500 && coreWebVitals.fid < 100 && coreWebVitals.cls < 0.1) {
        expect(result.data.overallScore).toBeGreaterThan(75);
      }
      
      // Poor Core Web Vitals should result in lower scores
      if (coreWebVitals.lcp > 4000 || coreWebVitals.fid > 300 || coreWebVitals.cls > 0.25) {
  ...
  describe('error handling', () => {
    it('should handle network timeouts gracefully', async () => {
      ...
      mockJob.data.params.websiteUrl = 'https://extremely-slow-website-12345.com';
    ...
    it('should handle invalid URLs gracefully', async () => {
      mockJob.data.params.websiteUrl = 'https://non-existent-domain-54321.com';
