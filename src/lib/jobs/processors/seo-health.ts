...
export class SEOHealthProcessor implements JobProcessor<SEOHealthJobData, SEOHealthResult> {
  ...
  async process(job: Job): Promise<JobResult<SEOHealthResult>> {
    try {
      const { websiteUrl, pages, includePerformance, includeMobile } = job.data.params;

      await this.updateProgress(job.id, 10, 'Starting SEO health assessment...');

      // Step 1: Technical infrastructure analysis (35% weight)
      await this.updateProgress(job.id, 20, 'Analyzing technical infrastructure...');
      const technicalScore = await this.analyzeTechnicalInfrastructure(websiteUrl, pages);

      // Step 2: On-page optimization analysis (30% weight)
      await this.updateProgress(job.id, 40, 'Evaluating on-page optimization...');
      const onPageScore = await this.analyzeOnPageOptimization(websiteUrl, pages);

      // Step 3: Performance analysis (20% weight - if enabled)
      let performanceScore = 85; // Default score
      if (includePerformance) {
        await this.updateProgress(job.id, 60, 'Analyzing performance metrics...');
      ...
      if (includeMobile) {
        await this.updateProgress(job.id, 75, 'Checking mobile optimization...');
      ...
      const result: SEOHealthResult = {
        ...
        recommendations,
        competitorComparison: await this.generateCompetitorComparison(overallScore),
      };
      ...
      await this.updateProgress(job.id, 100, 'SEO health assessment completed!');
  ...
  private async analyzeTechnicalInfrastructure(websiteUrl: string, pages: string[]): Promise<{
    score: number;
    issues: SEOIssue[];
  }> {
    ...
    maxScore += 35;
    const architectureAnalysis = await this.analyzeSiteArchitecture(websiteUrl, pages);
  ...
  private async analyzePageSpeed(_url: string): Promise<{ score: number; issues: SEOIssue[] }> {
    const issues: SEOIssue[] = [];
    ...
    try {
      ...
      if (mockSpeedScore < 70) {
        issues.push({
          ...
          howToFix: 'Optimize images, minify CSS/JS, enable compression, use a CDN.',
        });
      }
      ...
      if (mockSpeedScore < 80) {
        issues.push({
          ...
          impact: 'medium',
          fixComplexity: 'medium',
          howToFix: 'Improve Largest Contentful Paint, First Input Delay, and Cumulative Layout Shift.',
        });
      }
  ...
  private async analyzeMobileResponsiveness(_url: string): Promise<{ score: number; issues: SEOIssue[] }> {
    const issues: SEOIssue[] = [];
    ...
    try {
      ...
      if (mockMobileScore < 85) {
        issues.push({
          ...
          impact: 'medium',
          fixComplexity: 'easy',
          howToFix: 'Ensure responsive design, proper viewport meta tag, and touch-friendly elements.',
        });
      }
  ...
  private async analyzeSiteArchitecture(url: string, pages: string[]): Promise<{ score: number; issues: SEOIssue[] }> {
    const issues: SEOIssue[] = [];
    ...
    try {
      ...
      if (!sitemapCheck.exists) {
        ...
        issues.push({
          ...
          impact: 'medium',
          fixComplexity: 'easy',
          howToFix: 'Generate and submit an XML sitemap to help search engines discover your pages.',
        });
      }
  ...
  private async checkRobotsTxt(_url: string): Promise<{ exists: boolean; content?: string }> {
    try {
  ...
  private analyzeURLStructure(pages: string[]): { score: number; issues: SEOIssue[] } {
    const issues: SEOIssue[] = [];
  ...
  private async analyzeOnPageOptimization(websiteUrl: string, pages: string[]): Promise<{
    score: number;
    issues: SEOIssue[];
  }> {
    const issues: SEOIssue[] = [];
    ...
    const avgScore = samplesToAnalyze.length > 0 ? totalScore / samplesToAnalyze.length : 0;
  ...
  private async analyzePageOptimization(_url: string): Promise<{ score: number; issues: SEOIssue[] }> {
    const issues: SEOIssue[] = [];
    let score = 0;
    let maxScore = 0;
  ...
  private analyzeMetaDescription(html: string): { score: number; issues: SEOIssue[] } {
    const issues: SEOIssue[] = [];
    let score = 0;
  ...
  private analyzeHeadingStructure(html: string): { score: number; issues: SEOIssue[] } {
    const issues: SEOIssue[] = [];
    let score = 0;
  ...
  private analyzeInternalLinking(html: string, baseUrl: string): { score: number; issues: SEOIssue[] } {
    const issues: SEOIssue[] = [];
    let score = 0;
    ...
    try {
      ...
      if (internalLinks === 0) {
        ...
        issues.push({
          ...
          impact: 'low',
          fixComplexity: 'easy',
          howToFix: 'Add 3-5 relevant internal links to improve site navigation.',
        });
        score = 15;
      } else {
  ...
  private generateSEORecommendations(
    issues: SEOIssue[],
    scores: { technical: number; onPage: number; performance: number; mobile: number }
  ): SEORecommendation[] {
    ...
    for (const issue of sortedIssues.slice(0, 10)) { // Top 10 issues
      recommendations.push({
        ...
        resources: this.getRequiredResources(issue.category, issue.fixComplexity),
      });
    }
  ...
  private mapComplexityToNumber(complexity: 'easy' | 'medium' | 'hard'): number {
  ...
  private async generateCompetitorComparison(overallScore: number): Promise<SEOComparison> {
    // In production, this would analyze actual competitor data
    return {
      averageScore: 72,
      topPerformerScore: 89,
      yourPosition: overallScore > 72 ? Math.floor(Math.random() * 30) + 1 : Math.floor(Math.random() * 50) + 30,
      improvementPotential: Math.max(0, 89 - overallScore),
    };
  }

  private async storeResults(projectId: string, jobId: string, result: SEOHealthResult): Promise<void> {
    try {
