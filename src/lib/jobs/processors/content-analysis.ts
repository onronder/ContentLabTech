...
export class ContentAnalysisProcessor implements JobProcessor<ContentAnalysisJobData, ContentQualityResult> {
  ...
  async process(job: Job): Promise<JobResult<ContentQualityResult>> {
    ...
    try {
      const { websiteUrl, targetKeywords, competitorUrls, analysisDepth } = job.data.params;
      ...
      await this.updateProgress(job.id, 70, 'Analyzing readability and structure...');
      ...
      const semanticRelevance = await this.analyzeSemanticRelevance(websiteContent, targetKeywords);
      ...
      if (competitorUrls && competitorUrls.length > 0) {
        ...
        try {
          ...
          console.warn('Competitor analysis failed, continuing without:', error);
      ...
      const processingError = createProcessingError('content-analysis', error as Error, context);
  ...
  private async extractWebsiteContent(url: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        links: {
          internal: (html.match(/href=["'][^"']*["']/g) || []).filter(link => 
            !link.includes('http') || link.includes(new URL(url).hostname)
          ).length,
          external: (html.match(/href=["']https?:\/\/[^"']*["']/g) || []).filter(link => 
  ...
  private async analyzeTechnicalSEO(content: {
    ...
    if (content.metaDescription) {
      if (content.metaDescription.length >= 120 && content.metaDescription.length <= 160) {
        score += 20;
      } else if (content.metaDescription.length >= 80 && content.metaDescription.length <= 200) {
        score += 12;
  ...
  private calculateReadabilityScore(content: { content: string; headings: { level: number; text: string }[] }): number {
  ...
  private async analyzeSemanticRelevance(
    ...
    let totalKeywords = targetKeywords.length;
  ...
  private calculateFinalScores(data: {
    ...
    const technicalSeoWeight = 0.30;
    const contentDepthWeight = 0.40;
    const readabilityWeight = 0.20;
    const semanticRelevanceWeight = 0.10;
  ...
  private generateTechnicalRecommendations(score: number): ContentRecommendation[] {
    if (score >= 90) return [];
    ...
    if (score < 80) {
      recommendations.push({
        ...
        description: 'Ensure title tags are between 30-60 characters for optimal display.',
        implementation: 'Review and rewrite title tags to be concise yet descriptive.',
  ...
  private generateContentRecommendations(score: number): ContentRecommendation[] {
    if (score >= 85) return [];
    ...
    if (score < 75) {
      recommendations.push({
        ...
        implementation: 'Research and add detailed sections, examples, and expert insights.',
  ...
  private generateReadabilityRecommendations(score: number): ContentRecommendation[] {
    if (score >= 80) return [];
    ...
    recommendations.push({
      ...
      implementation: 'Use shorter sentences, bullet points, and logical heading hierarchy.',
      expectedImprovement: 15,
    });
  ...
  private generateSemanticRecommendations(score: number): ContentRecommendation[] {
    if (score >= 85) return [];
    ...
    recommendations.push({
      ...
      description: 'Better integrate target keywords naturally throughout content.',
  ...
  private async storeResults(projectId: string, jobId: string, result: ContentQualityResult): Promise<void> {
    try {
      const { error } = await this.supabase.from('content_analysis_results').insert({
        job_id: jobId,
