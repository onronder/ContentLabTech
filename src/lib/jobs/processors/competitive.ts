...
export class CompetitiveIntelligenceProcessor implements JobProcessor<CompetitiveIntelligenceJobData, CompetitiveIntelligenceResult> {
  ...
  async process(job: Job): Promise<JobResult<CompetitiveIntelligenceResult>> {
    try {
      ...
      await this.updateProgress(job.id, 50, 'Processing competitive analysis...');
  ...
  estimateProcessingTime(_data: CompetitiveIntelligenceJobData): number {
  ...
  private async storeResults(projectId: string, jobId: string, result: CompetitiveIntelligenceResult): Promise<void> {
    try {
