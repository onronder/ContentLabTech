...
export class StatisticalEngine {
  ...
  public calculatePearsonCorrelation(
    ...
    // Calculate means
    const meanX =
      (validX?.reduce((sum, val) => (sum || 0) + (val || 0), 0) || 0) / n;
    const meanY =
      (validY?.reduce((sum, val) => (sum || 0) + (val || 0), 0) || 0) / n;
    ...
    for (let i = 0; i < n; i++) {
      const xDev = (validX[i] || 0) - meanX;
      const yDev = (validY[i] || 0) - meanY;
    ...
    return {
      ...
      sampleSize: n,
      methodology:
        "Pearson product-moment correlation with Fisher transformation",
  ...
  public performLinearRegression(
    ...
    for (let i = 0; i < n; i++) {
      const xDev = (x[i] || 0) - meanX;
      const yDev = (y[i] || 0) - meanY;
    ...
    const predictions = x.map(xi => slope * (xi || 0) + intercept);
  ...
  public calculateReadabilityScore(
    ...
    switch (method) {
      ...
      case "coleman_liau":
        ...
        // Coleman-Liau Index
        const cliScore =
          0.0588 * (((avgCharactersPerWord * 100) / wordCount) * 100) -
    ...
    const confidence = this.calculateReadabilityConfidence(
      wordCount,
      sentenceCount
    );
  ...
  private createEmptyCorrelationResult(
  ...
  private calculateReadabilityConfidence(
    wordCount: number,
