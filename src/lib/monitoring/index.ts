...
export { errorTracker, trackError, setupGlobalErrorHandling, withErrorTracking } from './error-tracker';
 ...
 */
export function setupBasicMonitoring(options: {
  ...
  logger.updateConfig({
    level: logLevel === 'DEBUG' ? 0 : 
           logLevel === 'INFO' ? 1 :
           logLevel === 'WARN' ? 2 :
           logLevel === 'ERROR' ? 3 : 4
  });

  logger.info('Basic monitoring setup initiated', {
    enableHealthChecks,
    enableMetrics,
    enableErrorTracking,
  ...
  if (enableErrorTracking) {
    ...
    logger.info('Global error handling enabled', {}, ['setup', 'error-tracking']);
  }

  // Setup monitoring integration
  setupMonitoringIntegration();

  logger.info('Basic monitoring setup completed', {}, ['setup', 'completed']);
...
export const monitoringUtils = {
   ...
   */
  logPerformance: (operation: string, duration: number, success: boolean = true, details?: any) => {
    logger.performance({
      operation,
      duration,
      success,
   ...
   */
  logSecurity: (type: 'auth' | 'access' | 'suspicious' | 'attack', action: string, details?: any) => {
    logger.security({
      type,
      action,
      details
   ...
   */
  logBusiness: (type: string, action: string, userId?: string, details?: any) => {
    logger.business({
      type,
      action,
      userId,
...
export const devHelpers = process.env.NODE_ENV === 'development' ? {
  ...
  generateTestData: () => {
    ...
    logger.info('Test data generated for development', {}, ['dev-helpers']);
  },
  ...
  clearAllData: () => {
    ...
    logger.info('All monitoring data cleared for development', {}, ['dev-helpers']);
  },
